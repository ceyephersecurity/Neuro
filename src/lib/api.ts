...
export const githubApi = {
  ...
  deleteRepo: (owner: string, repo: string) =>
    api.delete(`/repos/${owner}/${repo}`).then(res => res.data),
  pushFiles: (owner: string, repo: string, data: { message: string; files: { path: string; content: string }[]; branch?: string }) =>
    api.post<{ success: boolean; commitSha: string }>(`/repos/${owner}/${repo}/push`, data).then(res => res.data),
